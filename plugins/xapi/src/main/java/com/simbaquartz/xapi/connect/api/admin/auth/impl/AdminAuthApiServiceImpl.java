        ...
        if (httpHeaders.getRequestHeaders().containsKey(API_KEY)){
            ...
            if (UtilValidate.isNotEmpty(apiKey)){
                try {
                    ...
                    //byte[] bytes = Base64.decode(apiKey);
                    ...
                    if(UtilValidate.isNotEmpty(userLogin)){
                        ...
                        Map fsdGetCustomerResponse;

                        fsdGetCustomerResponse = dispatcher.runSync("getAllPartyDetails", fsdGetAllPartyDetailsCtx);
                        ...
                        Debug.logError("Unable to retrieve login details for the logged in user. Please validate the apiKey header value.", module);
                        return ApiResponseUtil.prepareDefaultResponse(Response.Status.BAD_REQUEST, ApiMessageConstants.MSG_VALID_APIKEY_VALUE);
                    }
                } catch (Exception e) {
            ...
            return ApiResponseUtil.prepareDefaultResponse(Response.Status.BAD_REQUEST, ApiMessageConstants.MSG_MISSING_APIKEY_KEY);
        }

...
public class AuthenticationFilter implements ContainerRequestFilter {
  ...
  private static int TOKEN_INACTIVITY_MINS =
      ...
      if (UtilValidate.isNotEmpty(accessToken)) {
        try {
          ...
          if (UtilValidate.isEmpty(jwtMap)) {
            Debug.logError(
                "Unable to authorise the logged in user. Please validate the access token header value.",
                module);
            requestContext.abortWith(
                ApiResponseUtil.prepareDefaultResponse(
                    Response.Status.UNAUTHORIZED,
                    ApiMessageConstants.MSG_VALID_ACCESS_TOKEN_VALUE));
          } else {
            String tokenType = (String) jwtMap.get("tokenType");
            if (UtilValidate.isEmpty(tokenType) || !"ACCESS".equalsIgnoreCase(tokenType)) {
              Debug.logError(
                  "Invalid Access Token, please ensure you are not passing in a refresh token instead.",
                  module);
              requestContext.abortWith(
                  ApiResponseUtil.prepareDefaultResponse(
                      Response.Status.UNAUTHORIZED,
              ...
              if (EntityUtil.isMultiTenantEnabled() && UtilValidate.isNotEmpty(tenantId)) {
                tenantDelegator =
                    (GenericDelegator) DelegatorFactory.getDelegator("default#" + tenantId);
                ...
                tenantDispatcher =
                    new GenericDispatcherFactory()
                        .createLocalDispatcher("default#" + tenantId, tenantDelegator);
              }

              // Check if token is still valid
              boolean isTokenUuidValid =
                  checkIfAccessTokenUuidIsValid(tenantDelegator, userLoginId, accessTokenUuid);
              if (!isTokenUuidValid) {
          ...
          Debug.logError(
              e,
              "An Error occurred while trying to validate the access token: " + e.getMessage(),
              module);
        }
      } else {
        ...
        requestContext.abortWith(
            ApiResponseUtil.prepareDefaultResponse(
                Response.Status.UNAUTHORIZED, ApiMessageConstants.MSG_MISSING_ACCESS_TOKEN_VALUE));
      }
    } else {
      String pathRequested = requestContext.getUriInfo().getRequestUri().getPath();
      Debug.logError(
          "Access Token header key is missing for requested URL: " + pathRequested, module);
      requestContext.abortWith(
          ApiResponseUtil.prepareDefaultResponse(
              Response.Status.UNAUTHORIZED, ApiMessageConstants.MSG_MISSING_ACCESS_TOKEN_KEY));
    }
  }

...
inventoryCountItemAndVarList = from("InventoryCountItemsAndVariance").where(cond).orderBy(["locationSeqId ASC", "inventoryItemId ASC"]).queryList();
...
inventoryCountItemAndVarList.each { inventoryCountItemAndVar ->
    ...
    cycleCountMap.varianceQuantityOnHand = inventoryCountItemAndVar?.varianceQuantityOnHand;
    cycleCountMap.systemQuantityOnHand = inventoryCountItemAndVar?.systemQuantityOnHand;
    cycleCountMap.createBy = PartyHelper.getPartyName(delegator, inventoryCountItemAndVar.createdByUserLogin, false);

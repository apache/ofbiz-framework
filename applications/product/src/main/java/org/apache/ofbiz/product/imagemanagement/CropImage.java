    ...
    public static Map<String, Object> imageCrop(DispatchContext dctx, Map<String, ? extends Object> context)
        ...
        if (UtilValidate.isNotEmpty(imageName)) {
            ...
            if (!resolvedProductDir.startsWith(imageServerNormalizedPath)) {
                Debug.logError("Path traversal attempt detected in image crop, productId: " + productId, MODULE);
            ...
            if (!ImageManagementServices.isValidProductImageFileName(imageName, resolvedProductDir, delegator)) {
                Debug.logError("Path traversal attempt detected in image crop, imageName: " + imageName, MODULE);
                return ServiceUtil.returnError(UtilProperties.getMessage(RESOURCE,
